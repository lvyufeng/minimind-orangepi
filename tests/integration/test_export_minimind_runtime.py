...
def test_export_minimind_moe_runtime_from_safetensors(tmp_path: Path) -> None:
    root = Path(__file__).resolve().parents[2]
    model_dir = tmp_path / "moe_model"
    out_dir = tmp_path / "moe_runtime"
    model_dir.mkdir()
    config = {
        "hidden_size": 4,
        "num_hidden_layers": 1,
    ...
    tensors = {
        ...
        "model.layers.0.self_attn.v_proj.weight": ([2, 4], values(8, 0.01)),
        "model.layers.0.self_attn.o_proj.weight": ([4, 4], values(16, 0.01)),
        "model.layers.0.mlp.gate.weight": ([2, 4], values(8, 0.01)),
        "model.layers.0.mlp.experts.0.gate_proj.weight": ([8, 4], values(32, 0.01)),
        "model.layers.0.mlp.experts.0.up_proj.weight": ([8, 4], values(32, 0.01)),
        "model.layers.0.mlp.experts.0.down_proj.weight": ([4, 8], values(32, 0.01)),
        "model.layers.0.mlp.experts.1.gate_proj.weight": ([8, 4], values(32, 0.02)),
        "model.layers.0.mlp.experts.1.up_proj.weight": ([8, 4], values(32, 0.02)),
        "model.layers.0.mlp.experts.1.down_proj.weight": ([4, 8], values(32, 0.02)),
    }
    write_fake_safetensors(model_dir / "model.safetensors", tensors)
    ...
    config_text = (out_dir / "minimind_runtime_config.txt").read_text()
    assert "use_moe=1" in config_text
    assert "num_experts=2" in config_text
